...
def de(dg, dt, dc, bs=0, nth=1, single=0, **ka):
	...
	if single in {0, 1}:
		if bs == 0:
			bs = 500
		it = itertools.product(map(lambda x: [x, min(x + bs, ng)], range(0, ng, bs)),
							   map(lambda x: [x, min(x + bs, nt)], range(0, nt, bs)))
		if single == 0:
			...
			it = map(
				lambda x: [
					association_test_1,
					(x[0][0], x[1][0], dg[x[0][0]:x[0][1]], dt[x[1][0]:x[1][1]], dc,
					 dci, dcr), ka0], it)
		elif single == 1:
			it = map(
				lambda x: [
		...
		if bs == 0:
			...
			raise NotImplementedError(
				'Only allowing default setting (bs=0) for single=4')
		# Compute matrix products
		t1 = np.concatenate([dg, dc], axis=0)
		it = itertools.product(
			map(lambda x: [x, min(x + bs1, ng + nc)], range(0, ng + nc, bs1)),
			map(lambda x: [x, min(x + bs1, ng + nc)], range(0, ng + nc, bs1)))
		...
		it = itertools.product(
			map(lambda x: [x, min(x + bs1, ng + nc)], range(0, ng + nc, bs1)),
			map(lambda x: [x, min(x + bs1, nt)], range(0, nt, bs1)))
		it = map(
			lambda x: [
				prod1, (x[0][0], x[1][0], t1[x[0][0]:x[0][1]], dt[x[1][0]:x[1][1]]),
				dict()], it)
		ans0 = autopooler(nth, it, dummy=True)
		del t1
		...
		# Compute DE
		it = itertools.product(
			map(lambda x: [x, min(x + bs2, ng)], range(0, ng, bs2)),
			map(lambda x: [x, min(x + bs3, nt)], range(0, nt, bs3)))
		it = map(
			lambda x: [
				association_test_4,
				(x[0][0], x[1][0], tprod, tprody[:, x[1][0]:x[1][1]], tprodyy[x[
					1][0]:x[1][1]], [
						ng, x[1][1] - x[1][0], nc, ns, x[0][1] - x[0][0]]), ka0], it)
	...
	assert ans.shape == (dg0.shape[0], nt) and ansc.shape == (
		dg0.shape[0],
		nt) and ansa.shape == (dg0.shape[0], nt, nc) and ansvg.shape == (
			dg0.shape[0], ) and ansvt.shape == (dg0.shape[0], nt)
	assert np.isfinite(ans).all() and np.isfinite(ansc).all() and np.isfinite(
		ansa).all() and np.isfinite(ansvg).all() and np.isfinite(ansvt).all()
